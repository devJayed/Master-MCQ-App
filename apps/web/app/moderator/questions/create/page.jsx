...
const hasStimulusContent = (stimulus) =>
  Boolean(
    stimulus?.groupId?.trim() || stimulus?.content?.bn?.length || stimulus?.content?.en?.length
  );
...
export function QuestionEditor({ questionId, basePath = '/moderator/questions' }) {
    ...
    api(`/questions/${questionId}/manage`)
      .then((result) => {
        ...
        setForm({
          ...
          ...editorForm,
          questionType:
            editorForm.questionType === 'single_choice' || editorForm.questionType == null
              ? 0
              : Number(editorForm.questionType),
          ...
          optionContent: initialForm.optionContent.map(
            (fallback) =>
  ...
  const submit = async (status) => {
    ...
    for (const languageKey of ['bn', 'en']) {
      ...
      if (hasPlain && hasRich) {
        setMessage(
          `Use either plain or rich ${languageKey === 'bn' ? 'Bangla' : 'English'} question content, not both.`
        );
        return;
      }
      ...
      if (form.questionType === 0 && status === 'published' && !hasPlain && !hasRich) {
        setMessage(
          `Add ${languageKey === 'bn' ? 'Bangla' : 'English'} question content before publishing.`
        );
        return;
      }
    }
    ...
    <main className="mx-auto max-w-6xl p-5 md:p-10">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button
          ...
          onClick={() =>
            generate(
              form.questionType === 0 ? ['question', 'options', 'explanation'] : ['question']
            )
          }
          disabled={busy}
      ...
      >
        <section className="card border border-base-300 bg-base-100">
          <div className="card-body gap-7">
            <label className="form-control">
              <span className="label-text font-semibold">
                {text(language, 'প্রশ্নের ধরন', 'Question type')}
              </span>
              <select
              ...
              <span className="mt-1 text-xs text-base-content/50">
                {questionId
                  ? text(
                      language,
                      'তৈরির পরে ধরন পরিবর্তন করা যায় না।',
                      'Type cannot be changed after creation.'
                    )
                  : text(
                      language,
                      'ধরন অনুযায়ী প্রয়োজনীয় ঘর দেখানো হবে।',
                      'Required fields adapt to the selected type.'
                    )}
              </span>
            </label>
            ...
            {[0, 3, 4].includes(form.questionType) && (
              <details className="rounded-box border border-primary/20 bg-primary/5 p-4">
                <summary className="cursor-pointer font-semibold">
                  Shared stimulus / passage
                </summary>
                <p className="mt-2 text-xs text-base-content/60">
                  Use the same group ID on related questions. Add passage text, code, diagrams,
                  equations, or tables as ordered blocks.
                </p>
                <div className="mt-4 grid gap-3">
                  <label className="form-control">
                    ...
                    <input
                      ...
                      value={form.stimulus?.groupId || ''}
                      onChange={(event) =>
                        update('stimulus', { ...form.stimulus, groupId: event.target.value })
                      }
                      placeholder="e.g. hsc-ict-db-rb-2024-stimulus-1"
                    />
                  </label>
                  <LocalizedField
                    label="Stimulus instruction"
                    language={language}
                    value={FIXED_STIMULUS_TITLE}
                    onChange={() => {}}
                    multiline
                    readOnly
                  />
                  <RichContentEditor
                    ...
                    value={form.stimulus?.content}
                    onChange={(content) => update('stimulus', { ...form.stimulus, content })}
                  />
                </div>
              </details>
            )}
            <div>
              ...
              <details className="mt-3">
                <summary className="cursor-pointer text-sm font-semibold text-primary">
                  Use rich question body
                </summary>
                <p className="my-2 text-xs text-base-content/60">
                  Use either plain text or rich content for each language. Rich question blocks map
                  directly to Question Rich BN and Question Rich EN in Excel.
                </p>
                <RichContentEditor
                  label="Question body"
              ...
              <fieldset>
                <div className="mb-2 flex justify-between">
                  <legend className="font-semibold">
                    {text(language, 'বিকল্পসমূহ', 'Options')}
                  </legend>
                  <button
                    type="button"
                  ...
                  {form.options.map((option, index) => (
                    <div className="rounded-box border border-base-300 p-3" key={option.key}>
                      ...
                      <LocalizedField
                        ...
                        required={
                          !hasRichLanguage(
                            form.optionContent?.find((item) => item.key === option.key)?.content,
                            'bn'
                          )
                        }
                      />
                      <details className="mt-2">
                        ...
                        <RichContentEditor
                          ...
                          onChange={(content) => updateOptionContent(option.key, content)}
                        />
                      </details>
                    </div>
                  ))}
            ...
            {form.questionType === 0 ? (
              <div>
                ...
                <LocalizedField
                  ...
                  required={!hasRichLanguage(form.explanationContent, 'bn')}
                  multiline
                />
                <details className="mt-3">
                  <summary className="cursor-pointer text-sm font-semibold text-primary">
                    Add rich explanation content
                  </summary>
                  <RichContentEditor
                    label="Explanation"
                    value={form.explanationContent}
                    onChange={(value) => update('explanationContent', value)}
                  />
                </details>
              </div>
            ) : (
              <div>
                ...
                <details className="mt-3">
                  <summary className="cursor-pointer text-sm font-semibold text-primary">
                    {text(language, 'সমৃদ্ধ উত্তর ব্যবহার করুন', 'Use rich answer')}
                  </summary>
                  <RichContentEditor
                    label="Answer"
          ...
          <section className="card border border-base-300 bg-base-200">
            <div className="card-body p-5">
              ...
              {form.questionType === 0 &&
                form.options.map((option) => (
                  <div className="mt-2 text-sm" key={option.key}>
                    <b>{option.key}.</b>{' '}
                    <RichContent
                      content={form.optionContent?.find((item) => item.key === option.key)?.content}
              ...
              <div className="mt-4 border-t border-base-300 pt-3 text-xs">
                <b>
                  {form.questionType === 0
                    ? text(language, 'ব্যাখ্যা', 'Explanation')
                    : text(language, 'উত্তর', 'Answer')}
                </b>
                <RichContent
                  className="mt-1"
                  content={form.questionType === 0 ? form.explanationContent : form.answerContent}
                  fallback={form.questionType === 0 ? form.explanation : form.answer}
                  language={language}
                />
              </div>
            </div>
