...
const { createSubtopic } = require('../services/subtopic.service');
...
router.post('/', ...staff, async (req, res, next) => {
  try {
    ...
    if (!chapter || !topic || String(topic.chapterId) !== String(chapter._id))
      return res
        .status(400)
        .json({ message: 'Select a topic belonging to the selected active chapter.' });
    res.status(201).json({ data: await createSubtopic(req.body) });
  } catch (error) {
...
router.patch('/reorder', ...staff, async (req, res, next) => {
  try {
    ...
    const subtopics = await Subtopic.find({ _id: { $in: items.map((item) => item.id) } }).select(
      'topicId'
    );
    if (
      subtopics.length !== items.length ||
      new Set(subtopics.map((subtopic) => String(subtopic.topicId))).size !== 1
    )
    ...
    res.json({
      data: await Subtopic.find({ topicId: subtopics[0].topicId, isActive: true }).sort('order'),
    });
  } catch (error) {
    next(error);
  }
...
router.delete('/:id', ...staff, async (req, res, next) => {
  try {
    ...
    if (await Question.exists({ subtopicId: subtopic._id, isDeleted: false })) {
      ...
      return res.json({
        data: { archived: true },
        message: 'Subtopic was archived because questions reference it.',
      });
    }
